...
data={
    "Hour":[1,2,3,4,5,1,2,3,9,4,3,2,2,1],
...
cat_cols=["AREA NAME","Crm Cd Desc","Vict Sex","Vict Descent","Premis Desc","Weapon Desc","Status Desc","DayOfWeek","TimeOfDay"]
X=pd.get_dummies(df.drop(columns=["Hour"]),columns=cat_cols,drop_first=True)
X.columns=X.columns.str.replace(r"[\[\], ]+","_",regex=True).str.replace(r"[^a-zA-Z0-9_]","",regex=True).str.strip("_")
y=df["Hour"]
